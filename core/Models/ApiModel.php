<?php
 ...
 // Calculate Recharge Pin Discount
	...
	//validate Billstack Notification
		public function verifyBillstackNotification($key,$email){
            ...
            $bilstackKey = $this->getConfigValue($result,"bilstackApi");
            
            $bilstackCharges = (float) $this->getConfigValue($result,"bilstackCharges");
            $bilstackChargesType = $this->getConfigValue($result,"bilstackChargesType");
            
            //Verify Notification Details
            if($key == md5($bilstackKey)):
                ...
                if($queryA->rowCount() > 0):
                    ...
                    $response["charges"] = $bilstackCharges;
                    $response["chargestype"] = $bilstackChargesType;
		...
		public function getNumberOfAvailablePins(){
			...
			$accessType = $this->getConfigValue($details,"rechargePinMethod");
