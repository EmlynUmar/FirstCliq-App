...
// Get	Sales By Users.
		public function getSpent($limit){
			...
			$data=$this->model->getSpent($limit,$datefrom,$dateto);
			return $data;
		}
		...
		public function updateNetworkSetting(){
			extract($_POST);
			$check=$this->model->updateNetworkSetting($network,$general,$vtuStatus,$sharesellStatus,$airtimepin,$datapin,$sme,$sme2,$gifting,$corporate,$dataflex,$awoof,$share,$networkid,$vtuId,$sharesellId,$smeId,$sme2Id,$giftingId,$corporateId,$dataflexId,$awoofId,$shareId);
		...
		public function updateSiteSetting(){
			extract($_POST);
			$check=$this->model->updateSiteSetting($sitename,$siteurl,$apidocumentation,$referalupgradebonus,$referalairtimebonus,$referaldatabonus,$referalwalletbonus,$referalcablebonus,$referalexambonus,$referalmeterbonus,$wallettowalletcharges,$agentupgrade,$vendorupgrade,$apiupgrade,$accountname,$accountno,$bankname,$electricitycharges,$airtimemin,$airtimemax,$smilediscount,$kycoption,$kycbvncharges,$kycnincharges,$kycShouldEnable,$kycShouldVerify);
		...
		public function addAirtimeDiscount(){
			extract($_POST);
			$check=$this->model->addAirtimeDiscount($network,$networktype,$buydiscount,$userdiscount,$agentdiscount,$vendordiscount,$apidiscount);
		...
		public function updateAirtimeDiscount(){
			extract($_POST);
			$check=$this->model->updateAirtimeDiscount($networkid,$network,$networktype,$buydiscount,$userdiscount,$agentdiscount,$vendordiscount,$apidiscount);
		...
		public function addAlphaTopup(){
			extract($_POST);
			$check=$this->model->addAlphaTopup($bprice,$sprice,$agent,$vendor,$api);
		...
		public function updateAlphaTopup(){
			extract($_POST);
			$check=$this->model->updateAlphaTopup($alphaid,$buying,$selling,$agent,$vendor,$api);
		...
		public function addRechargeCardPinDiscount(){
			extract($_POST);
			$check=$this->model->addRechargeCardPinDiscount($network,$amount,$buyprice,$userdiscount,$agentdiscount,$vendordiscount,$apidiscount,$loadpin,$checkbal,$planid);
			if($check == 0){return $this->createNotification1("alert-success","Discount Added Successfully");}
			elseif($check == 2){return $this->createNotification1("alert-danger","Discount With Same Network Id Already Exist.");}
			else{return $this->createNotification1("alert-danger","Unable To Add New Discount, Please Try Again");}
		}
		...
		public function updateRechargeCardPinDiscount(){
			extract($_POST);
			$check=$this->model->updateRechargeCardPinDiscount($network,$amount,$buyprice,$userdiscount,$agentdiscount,$vendordiscount,$apidiscount,$loadpin,$checkbal,$planid);
			if($check == 0){return $this->createNotification1("alert-success","Discount Updated Successfully");}
			else{return $this->createNotification1("alert-danger","Unable To Update Discount, Please Try Again");}
		}
		...
		public function addDataPlan(){
			extract($_POST);
			$check=$this->model->addDataPlan($network,$dataname,$datatype,$planid,$duration,$price,$userprice,$agentprice,$vendorprice,$apiprice);
			if($check == 0){return $this->createNotification1("alert-success","Data Plan Added Successfully");}
			elseif($check == 2){return $this->createNotification1("alert-danger","Plan With Same Id Already Exist.");}
			else{return $this->createNotification1("alert-danger","Unable To Add New Plan, Please Try Again");}
		}
		...
		public function updateDataPlan(){
			extract($_POST);
			$check=$this->model->updateDataPlan($plan,$network,$dataname,$datatype,$planid,$duration,$price,$userprice,$agentprice,$vendorprice,$apiprice);
			if($check == 0){return $this->createNotification1("alert-success","Plan Updated Successfully");}
			else{return $this->createNotification1("alert-danger","Unable To Update Plan, Please Try Again");}
		}
		...
		public function addDataPin(){
			extract($_POST);
			$check=$this->model->addDataPin($network,$dataname,$datatype,$planid,$duration,$price,$userprice,$agentprice,$vendorprice,$apiprice,$loadpin,$checkbal);
		...
		public function updateDataPin(){
			extract($_POST);
			$check=$this->model->updateDataPin($pin,$network,$dataname,$datatype,$planid,$duration,$price,$userprice,$agentprice,$vendorprice,$apiprice,$loadpin,$checkbal);
		...
		public function addCablePlan(){
			extract($_POST);
			$check=$this->model->addCablePlan($provider,$planname,$planid,$duration,$price,$userprice,$agentprice,$vendorprice,$apiprice);
		...
		public function updateCablePlan(){
			extract($_POST);
			$check=$this->model->updateCablePlan($plan,$provider,$planname,$planid,$duration,$price,$userprice,$agentprice,$vendorprice,$apiprice);
...
// Blacklist Number
  public function blacklistNumber() {
    ...
    // Check if the phone number is already blacklisted
    $isBlacklisted = $this->model->blacklistNumber($phone);
    
    if ($isBlacklisted === false) {
        return $this->createNotification1("alert-danger", "Phone number already blacklisted!");
    } else {
        return $this->createNotification1("alert-success", "Phone number successfully blacklisted.");
    }
}

  // Get Blacklisted Numbers
  public function getBlacklistNumber($limit) {
    $blacklist = $this->model->getBlacklistNumber($limit);
    return $blacklist;
   }

  // Delete Blacklisted Number
  public function deleteBlacklistNumber() {
    extract($_POST);
    $isDeleted = $this->model->deleteBlacklistNumber($id);
    
    if ($isDeleted > 0) {
        return $this->createNotification1("alert-success", "Phone number removed from blacklist."); } else {
        return $this->createNotification1("alert-danger", "Failed to remove phone number from blacklist.");}
       }
