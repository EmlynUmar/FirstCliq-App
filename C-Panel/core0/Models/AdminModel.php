<?php
	...
	//Update Site Setting
		public function updateSiteSetting($sitename,$siteurl,$apidocumentation,$referalupgradebonus,$referalairtimebonus,$referaldatabonus,$referalwalletbonus,$referalcablebonus,$referalexambonus,$referalmeterbonus,$wallettowalletcharges,$agentupgrade,$vendorupgrade,$apiupgrade,$accountname,$accountno,$bankname,$electricity,$airtimemin,$airtimemax,$smilediscount,$kycoption,$kycbvncharges,$kycnincharges,$kycShouldEnable,$kycShouldVerify){
			$dbh=$this->connect();
			$sql="UPDATE sitesettings SET sitename=:sn,siteurl=:u,agentupgrade=:au,vendorupgrade=:vu,apiupgrade=:ap,apidocumentation=:ad,referalupgradebonus=:rub,referalairtimebonus=:rab,referaldatabonus=:rdb,referalwalletbonus=:rwb,referalcablebonus=:rcb,referalexambonus=:reb,referalmeterbonus=:rmb,wallettowalletcharges=:wwc,accountname=:accna,accountno=:accno,bankname=:bnkna,electricitycharges=:electc,airtimemin=:amin,airtimemax=:amax,smilediscount=:smilediscount,kycOption=:kycO,kycBvnCharges=:kycB,kycNinCharges=:kycN,kycShouldEnable=:kycE,kycShouldVerify=:kycV WHERE sId=1";
			$query = $dbh->prepare($sql);
			...
			$query->bindParam(':ap',$apiupgrade,PDO::PARAM_STR);
		...
		public function updateNetworkSetting($network,$general,$vtuStatus,$sharesellStatus,$airtimepin,$datapin,$sme,$sme2,$gifting,$corporate,$dataflex,$awoof,$share,$networkid,$vtuId,$sharesellId,$smeId,$sme2Id,$giftingId,$corporateId,$dataflexId,$awoofId,$shareId){
			$dbh=$this->connect();
			$id = (float) $network;
			
			$sql="UPDATE networkid SET networkStatus=:g, vtuStatus=:vs, sharesellStatus=:sss, airtimepinStatus=:ap, datapinStatus=:dp, smeStatus=:s, sme2Status=:s2, giftingStatus=:gi, corporateStatus=:c,dataflexStatus=:d,awoofStatus=:aw,shareStatus=:c2, networkId=:nid, vtuId=:vtuid, sharesellId=:sharesellid,smeId=:smeid,sme2Id=:sme2id,giftingId=:giftid,corporateId=:ccid,dataflexId=:df,awoofId=:aw2,shareId=:cc2id WHERE nId = $id";
			$query = $dbh->prepare($sql); 
			...
			$query->bindParam(':d',$dataflex,PDO::PARAM_STR);
			$query->bindParam(':aw',$awoof,PDO::PARAM_STR);
			$query->bindParam(':c2',$share,PDO::PARAM_STR);
			...
			$query->bindParam(':sme2id',$sme2Id,PDO::PARAM_STR);
			$query->bindParam(':giftid',$giftingId,PDO::PARAM_STR);
			$query->bindParam(':ccid',$corporateId,PDO::PARAM_STR);
			$query->bindParam(':df',$dataflexId,PDO::PARAM_STR);
			$query->bindParam(':aw2',$awoofId,PDO::PARAM_STR);
			$query->bindParam(':cc2id',$shareId,PDO::PARAM_STR);
		...
		public function addAirtimeDiscount($network,$networktype,$buydiscount,$userdiscount,$agentdiscount,$vendordiscount,$apidiscount){
			$dbh=$this->connect();
			...
			$sql="INSERT INTO airtime(aNetwork,aType,aBuyDiscount,aUserDiscount,aAgentDiscount,aVendorDiscount,aApiDiscount) VALUES(:n,:ny,:b,:u,:a,:p,:v)";
            $query = $dbh->prepare($sql);
            ...
            $query->bindParam(':p',$apidiscount,PDO::PARAM_STR);
            $query->execute();
            $lastInsertId = $dbh->lastInsertId();
            if($lastInsertId){return 0;} else{return 1;}
		}
		...
		public function updateAirtimeDiscount($id,$network,$networktype,$buydiscount,$userdiscount,$agentdiscount,$vendordiscount,$apidiscount){
			$dbh=$this->connect();
			$id= (int) base64_decode($id);
			$sql="UPDATE airtime SET aNetwork=:n,aType=:nt,aBuyDiscount=:b,aUserDiscount=:u,aAgentDiscount=:a,aApiDiscount=:p,aVendorDiscount=:v WHERE aId=$id";
            $query = $dbh->prepare($sql);
            $query->bindParam(':n',$network,PDO::PARAM_STR);
            $query->bindParam(':nt',$networktype,PDO::PARAM_STR);
            $query->bindParam(':b',$buydiscount,PDO::PARAM_STR);
		...
		public function addRechargeCardPinDiscount($network,$userdiscount,$agentdiscount,$vendordiscount,$apidiscount){
			$dbh=$this->connect();
			...
			//If Not Exist, Create New Discount
			$sql="INSERT INTO airtimepinprice (aNetwork,aUserDiscount,aAgentDiscount,aApiDiscount,aVendorDiscount) VALUES(:n,:u,:a,:p,:v)";
            $query = $dbh->prepare($sql);
            $query->bindParam(':n',$network,PDO::PARAM_STR);
            $query->bindParam(':u',$userdiscount,PDO::PARAM_STR);
            $query->bindParam(':a',$agentdiscount,PDO::PARAM_STR);
		...
		public function updateRechargeCardPinDiscount($network,$userdiscount,$agentdiscount,$vendordiscount,$apidiscount){
			$dbh=$this->connect();
			$sql="UPDATE airtimepinprice SET aUserDiscount=:u,aAgentDiscount=:a,aApiDiscount=:p,aVendorDiscount=:v WHERE aNetwork=:n";
            $query = $dbh->prepare($sql);
            $query->bindParam(':n',$network,PDO::PARAM_STR);
            $query->bindParam(':u',$userdiscount,PDO::PARAM_STR);
            $query->bindParam(':a',$agentdiscount,PDO::PARAM_STR);
		...
		//Add Data Plans
		public function addDataPlan($network,$dataname,$datatype,$planid,$duration,$price,$userprice,$agentprice,$vendorprice,$apiprice){
			$dbh=$this->connect();
			...
			//If Not Exist, Create New User
			$sql="INSERT INTO  dataplans (datanetwork,name,type,planid,day,price,userprice,agentprice,vendorprice,apiprice) 
			VALUES(:n,:d,:dt,:p,:du,:pr,:up,:ap,:az,:vp)";
            $query = $dbh->prepare($sql);
            $query->bindParam(':n',$network,PDO::PARAM_STR);
            ...
            $query->bindParam(':az',$apiprice,PDO::PARAM_STR);
            $query->execute();
            $lastInsertId = $dbh->lastInsertId();
            if($lastInsertId){return 0;} else{return 1;}
		}
		...
		//Update Data Plan
		public function updateDataPlan($plan,$network,$dataname,$datatype,$planid,$duration,$price,$userprice,$agentprice,$vendorprice,$apiprice){
			$dbh=$this->connect();

			
			//If Not Exist, Create New User
			$sql="UPDATE dataplans SET datanetwork=:n,name=:d,type=:dt,planid=:p,day=:du,price=:pr,userprice=:up,agentprice=:ap,vendorprice=:vp,apiprice=:az WHERE pId=:id";
            $query = $dbh->prepare($sql);
		...
		//Add Data Pins
		public function addDataPin($network,$dataname,$datatype,$planid,$duration,$price,$userprice,$agentprice,$vendorprice,$apiprice){
			$dbh=$this->connect();
			...
			//If Not Exist, Create New User
			$sql="INSERT INTO  datapins (datanetwork,name,type,planid,day,price,userprice,agentprice,vendorprice,apiprice) 
			VALUES(:n,:d,:dt,:p,:du,:pr,:up,:ap,:az,:vp)";
            $query = $dbh->prepare($sql);
            $query->bindParam(':n',$network,PDO::PARAM_STR);
            $query->bindParam(':d',$dataname,PDO::PARAM_STR);
		...
		//Add Cable Plans
		public function addCablePlan($provider,$planname,$planid,$duration,$price,$userprice,$agentprice,$vendorprice,$apiprice){
			$dbh=$this->connect();
			...
			//If Not Exist, Create New User
			$sql="INSERT INTO cableplans (cableprovider,name,planid,day,price,userprice,agentprice,vendorprice,apiprice) 
			VALUES(:cp,:n,:p,:du,:pr,:up,:ap,:az,:vp)";
            $query = $dbh->prepare($sql);
		...
		//Update Cable Plan
		public function updateCablePlan($plan,$provider,$planname,$planid,$duration,$price,$userprice,$agentprice,$vendorprice,$apiprice){
			$dbh=$this->connect();

			//If Not Exist, Create New User
			$sql="UPDATE cableplans SET cableprovider=:p,name=:pn,planid=:pi,day=:du,price=:pr,userprice=:up,agentprice=:ap,apiprice=:az,vendorprice=:vp WHERE cpId=:id";
            $query = $dbh->prepare($sql);
            $query->bindParam(':id',$plan,PDO::PARAM_STR);
...
//Get All Processing Transactions
		...
		// Get	Sales By Users.
  function getSpent($limit,$datefrom,$dateto) {
    $dbh = $this->connect();
    $sql = "SELECT a.sEmail, a.sFname, a.sLname, SUM(b.profit) AS total_profit, SUM(b.amount) AS total_amount 
            FROM subscribers a 
            INNER JOIN transactions b ON a.sId = b.sId 
            WHERE b.status = 0
            AND b.servicename IN ('Data', 'Airtime')
            AND b.date BETWEEN ? AND ?
            GROUP BY a.sId 
            ORDER BY total_amount DESC 
            LIMIT ?";
    $query = $dbh->prepare($sql);
    $query->bindParam(1, $datefrom);
    $query->bindParam(2, $dateto);
  ...
  // Add a phone number to the blacklist
   public function blacklistNumber($phone) { 
      $dbh = $this->connect();
   // Check if the phone number already exists in the blacklist
      $sql_check = "SELECT COUNT(*) FROM blacklist WHERE bPhone = :phone";
    ...
    // Insert the phone number into the blacklist
        $sql_insert = "INSERT INTO blacklist (bPhone) VALUES (:phone)";
   ...
   // Retrieve all phone numbers from the blacklist
   public function getBlacklistNumber($limit) { 
      $dbh = $this->connect();
      $sql = "SELECT * FROM blacklist LIMIT $limit";
      $query = $dbh->prepare($sql);
      $query->execute();
   ...
   public function deleteBlacklistNumber($id) { 
    $dbh = $this->connect();
