        ...
        $("#airtimeamount").on("keyup",function(){
            ...
            for (i=0; i< airtimediscount.length; i++){
                if(airtimediscount[i].aNetwork == $("#networkid").val() && airtimediscount[i].aType == $("#networktype").val()){
                    ...
                    else if(useraccount == 4 || useraccount =='4'){discount=airtimediscount[i].aApiDiscount;}
...
//Purchase Airtime
        $("#verifyForm").submit(function(e){
            ...
            if($("#thetranspin").val() == null || $("#thetranspin").val() == ''){
                ...
                let msg = "You are about to verify a NIN ";
                msg+=''+$('#networkid').val().toUpperCase() + ' Slip ' + " at ";
                msg+='' + $('#networkid').find(":selected").attr('data-price') + '' + " in favor of " + '<b>' + $("#phone").val() +'</b>';
            ...
            $('#verify-nin').removeClass("gradient-highlight");
            $('#verify-nin').addClass("btn-secondary");
            $('#verify-nin').html('<i class="fa fa-spinner fa-spin" aria-hidden="true"></i> Processing ...');
     ...
     // ----------------------------------------------------------------------------
        ...
        $(document).ready(function() {
    // Initialize variables to track message length, sets, and phone numbers
    var prevMessageLength = 0;
    var setsCounted = 0;
    var firstPhoneNumberCounted = false;
    
    // Update the amounttopay field value as phone numbers or message are being inputted
    $("#phoneNumbers, #message").on("input", function() {
        var phoneNumbers = $("#phoneNumbers").val();
        var numPhoneNumbers = countPhoneNumbers(phoneNumbers);
        
        var message = $("#message").val();
        var numAlphabetCharacters = countAlphabetCharacters(message);
        
        // Check if the message length has increased by 160 characters
        if (numAlphabetCharacters - prevMessageLength >= 160) {
            prevMessageLength = numAlphabetCharacters;
            if (setsCounted === 0) {
                setsCounted++;
            } else {
                increasePrice();
        ...
        // Check if the first phone number is counted
        if (!firstPhoneNumberCounted && numPhoneNumbers > 1) {
            firstPhoneNumberCounted = true;
        }
        
        var totalAmountToPay = calculateAmountToPay(numPhoneNumbers, numAlphabetCharacters, setsCounted, firstPhoneNumberCounted);
        
        $("#amount").val(totalAmountToPay.toFixed(2));
    });

    $("#bulkSMSForm").submit(function(e) {
        if ($("#thetranspin").val() == null || $("#thetranspin").val() == '') {
            e.preventDefault();
            $("#transpinbtn").attr("action-btn", "btnsubmit");

            let msg = "You are about to send a bulk SMS with the following details:\n\n";
            msg += "Sender Name: " + $("#senderName").val() + "\n";
          
            msg += "Amount To Pay: ₦" + $("#amount").val() + "\n\n";
        ...
        $('#btnsubmit').removeClass("gradient-highlight");
        $('#btnsubmit').addClass("btn-secondary");
        $('#btnsubmit').html('<i class="fa fa-spinner fa-spin" aria-hidden="true"></i> Processing ...');
    });

    // Rest of your existing functions...
// Function to count the number of phone numbers
function countPhoneNumbers(phoneNumbers) {
    var phoneNumberArray = phoneNumbers.split(",");
    var count = 0;
    
    for (var i = 0; i < phoneNumberArray.length; i++) {
        var trimmedNumber = phoneNumberArray[i].trim();
        if (i === 0) { // Skip the first phone number
            continue;
        }
        if (trimmedNumber.match(/^\d{11}$/)) {
            count++;
        }
    }
    ...
    function countAlphabetCharacters(text) {
        var alphabetRegex = /[a-zA-Z]/g;
        var alphabetMatches = text.match(alphabetRegex);
        return alphabetMatches ? alphabetMatches.length : 0;
    }

    // Function to increase the price
    function increasePrice() {
        var currentAmount = parseFloat($("#amount").val());
        var increasedAmount = currentAmount + 3.5;
        $("#amount").val(increasedAmount.toFixed(2));
    }

    // Function to calculate the amount to pay based on the number of phone numbers, alphabet characters, sets counted, and first phone number counted
    function calculateAmountToPay(numPhoneNumbers, numAlphabetCharacters, setsCounted, firstPhoneNumberCounted) {
        var costPerPhoneNumber = 3.5; // Cost per phone number
        var costPerAlphabetCharacters = 3.5; // Cost per 160 alphabet characters
        var numMessageSegments = Math.max(Math.ceil((numAlphabetCharacters - 160 * setsCounted) / 160), 0);
        var totalCost = 0;

        if (firstPhoneNumberCounted) {
            totalCost = (numPhoneNumbers * costPerPhoneNumber) + (numMessageSegments * costPerAlphabetCharacters);
        } else {
            totalCost = (numMessageSegments * costPerAlphabetCharacters);
        }
            ...
            if($("#cableid").val() == '' || $("#cableid").val() == null){
                ...
                if(!plans == ""){
                    ...
                    for(i=0; i < plans.length; i++){
                        ...
                        else if(useraccount == 4 || useraccount =='4'){price = plans[i].apiprice;}
       ...
       $("#electricityForm").submit(function(e){
        ...
        $("#networkid").on("change",function(){
            ...
            let dataflex = $('#networkid').find(":selected").attr('dataflex');
            let awoof = $('#networkid').find(":selected").attr('awoof');
            ...
            let thegroup='<option value="">Select Type</option>';
                ...
                //Check If Network Is Disabled
                if(dataflex=="On"){
                   thegroup+='<option value="dataflex">Dataflex</option>';
                }
                
                //Check If Network Is Disabled
                if(awoof=="On"){
                   thegroup+='<option value="Awoof">Awoof Data</option>';
                }
        ...
        let useraccount = getCookie("loginAccount");
           ...
           useraccount = parseInt(useraccount);
            ...
            if(useraccount == 2){$("#amounttopay").val("N"+$('#alphaplan').find(":selected").attr('agent'));}
             if(useraccount == 4){$("#amounttopay").val("N"+$('#alphaplan').find(":selected").attr('api'));}
