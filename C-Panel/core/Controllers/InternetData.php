<?php
    ...
    class InternetData extends ApiAccess{
        ...
        if ($datagroup == "SME") {
        ...
        } elseif ($datagroup == "dataflex") {
            $name = "dataflex";
            $thenetworkId = $networkDetails["dataflexId"];
        } elseif ($datagroup == "Awoof") {
            $name = "Awoof";
            $thenetworkId = $networkDetails["awoofId"];
        } else {
            $name = "Share";
            $thenetworkId = $networkDetails["shareId"];
        }
            ...
            if(strpos($host, 'fatizara') !== false){
                $hostuserurl="https://fatizaradata.com/api/user/";
                return $this->purchaseDataWithBasicAuthentication($body,$host,$hostuserurl,$apiKey,$thenetworkId,$actualPlanId);
            }
            ...
            if (strpos($host, 'edata') !== false) {
            return $this->dataEdata($body, $host, $apiKey, $thenetworkId, $actualPlanId);
        }
        ...
        //Purchase Data From edata
		public function dataEdata($body, $host, $apiKey, $thenetworkId, $actualPlanId)
    {
        ...
        if ($err) {
            $response["status"] = "fail";
            $response["msg"] = "Server Connection Error: " . $err;
            //  file_put_contents("Edata_error_log2.txt", json_encode($response));
            curl_close($curl);
            return $response;
        }
        ...
        if (
            ...
            //file_put_contents("Edata.txt", json_encode($result));
        } else {
            $response["status"] = "fail";
            $response["msg"] = "Server/Network Error";
            ///file_put_contents("Edata_error_log.txt", json_encode($result));
        }
