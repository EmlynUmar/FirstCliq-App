	     	...
	     	$queryC->execute();
	      	...
	      	//Insert And Register Member
		       ...
		       if($lastInsertId){
					...
					$check=$this->sendMail($adminEmail,$subject2,$message2);
	    	...
	    	$queryC->bindParam(':ph',$phone,PDO::PARAM_STR);
	     	...
	     	$queryC->execute();
	      	...
	      	if($queryC->rowCount() > 0){
				...
				$check=$this->sendMail($result->sEmail,$subject,$message);
...
//create Aspfiy Virtual Account
        public function generateAsfiy($id,$email,$fname,$lname,$aspfiyApi,$phone,$aspfiyWebhook){
            ...
            $ref = str_shuffle(substr("ABCDEFGHJKLMNPQRSTUVWXYZabcdefghjklmnpqrstuvwxyz1234567890",0,12));
				...
				$queryC->execute();
                 ...
                 public function generatePaga($id,$email,$fname,$lname,$aspfiyApi,$phone,$aspfiyWebhook){
            $secondname= explode(" ",$lname);
	        if(isset($secondname[0])): $lname = $secondname[0]; endif;
            if(isset($secondname[1])): $mname = $secondname[1]; else: $mname =""; endif;
            $fname=str_replace(" ","",$fname); 
            ...
            $ref = str_shuffle(substr("ABCDEFGHJKLMNPQRSTUVWXYZabcdefghjklmnpqrstuvwxyz1234567890",0,12));
                ...
                $response = curl_exec($curl);
                $err = curl_error($curl);
                file_put_contents("paga",$response);
                curl_close($curl);
				...
				$dbh=$this->connect();
				$c="UPDATE subscribers SET sPagaBank=:AS WHERE sId=$id";
				$queryC = $dbh->prepare($c);
				$queryC->bindParam(':AS',$accountNumber,PDO::PARAM_STR);
				$queryC->execute();
			}
    ...
    //Create Dynamic SafeHaven
         //create Aspfiy Virtual Account
        public function generateDynamicSafeHaven($id,$email,$fname,$lname,$aspfiyApi,$phone,$aspfiyWebhook){
            $secondname= explode(" ",$lname);
	        if(isset($secondname[0])): $lname = $secondname[0]; endif;
            if(isset($secondname[1])): $mname = $secondname[1]; else: $mname =""; endif;
            $fname=str_replace(" ","",$fname); 
            ...
            $ref = str_shuffle(substr("ABCDEFGHJKLMNPQRSTUVWXYZabcdefghjklmnpqrstuvwxyz1234567890",0,12));
                ...
                curl_setopt_array($curl, [
                  CURLOPT_URL => "https://api-v1.aspfiy.com/reserve-dynamic-account/",
                  CURLOPT_RETURNTRANSFER => true,
                  CURLOPT_ENCODING => "",
                  CURLOPT_MAXREDIRS => 10,
                  CURLOPT_TIMEOUT => 30,
                ...
                $response = curl_exec($curl);
                $err = curl_error($curl);
                file_put_contents("aspfiy_gen_dyn",$response);
                curl_close($curl);
                ...
                if(isset($res->data->account->account_number)){
                $accountNumber = $res->data->account->account_number;
                $accountExpiry = $res->data->account->account_expiry;
                
				//Save Account Number
				
				$dbh=$this->connect();
				$c="UPDATE subscribers SET sDynamicBank=:AS, sDynamicBankExpiry=:BE WHERE sId=$id";
				$queryC = $dbh->prepare($c);
				$queryC->bindParam(':AS',$accountNumber,PDO::PARAM_STR);
				$queryC->bindParam(':BE',$accountExpiry,PDO::PARAM_STR);
				$queryC->execute();
 ...
 //create billstack virtual account
        ...
        public function generateBillstack($id,$email,$fname,$lname,$billstackApi,$phone,$bank){
            $secondname= explode(" ",$lname);
	        if(isset($secondname[0])): $lname = $secondname[0]; endif;
            if(isset($secondname[1])): $mname = $secondname[1]; else: $mname =""; endif;
            $fname=str_replace(" ","",$fname); 
            ...
            $ref = str_shuffle(substr("ABCDEFGHJKLMNPQRSTUVWXYZabcdefghjklmnpqrstuvwxyz1234567890",0,12));
                ...
                curl_setopt_array($curl, [
                  ...
                  CURLOPT_HTTPHEADER => [
                    "Authorization: Bearer {$billstackApi}",
                    "Content-Type: application/json",
                    "accept: application/json"
                  ],
                ]);
                ...
                $response = curl_exec($curl);
                $err = curl_error($curl);
                file_put_contents("billstack_gen",$response);
                curl_close($curl);
                
                $res = json_decode($response);
                
                if(isset($res->data->account[0]->account_number)){
                $accountNumber = $res->data->account->account_number;
				...
				$dbh=$this->connect();
				$c="UPDATE subscribers SET sBilstackBank=:AS WHERE sId=$id";
				$queryC = $dbh->prepare($c);
				$queryC->bindParam(':AS',$accountNumber,PDO::PARAM_STR);
				$queryC->execute();
			}
