...
//Get Bulk Sms
	 ...
	 //Purchase Rechargepin 
       ...
       public function getRechargePinTokens($id,$ref){
		...
		//Upgrade To Api
		public function upgradeToApi($userId,$pin,$ref){
			$dbh=$this->connect();
			$sql = "SELECT sType,sFname,sLname,sPhone,sWallet,sPin,sReferal FROM subscribers WHERE sId=:id";
            $query = $dbh->prepare($sql);
			$query->bindParam(':id',$userId,PDO::PARAM_INT);
            ...
            $results=$query->fetch(PDO::FETCH_OBJ);
			$result2 = $this->getSiteSettings();
			$amount = (float) $result2->apiupgrade;
			...
			if($results->sPin == $pin || $pinstatus == 1){
				if($results->sType == 3){return 2;}
					...
					if($balance >= $amount){
            			...
            			$newbalance = $oldbalance - $amount;
						$servicename = "Account Upgrade";
    					$servicedesc = "Upgraded Account Type To Api Account.";
						$status = 0;
						$date=date("Y-m-d H:i:s");
		...
		public function verifyTransactionPin($userId,$transkey){
			...
			$sql = "SELECT sPinStatus,sPin,sApiKey,sWallet,sType FROM subscribers WHERE sId=:id";
			$query = $dbh->prepare($sql);
			$query->bindParam(':id',$userId,PDO::PARAM_INT);
			$query->execute();
			...
			if($user){
				if($user->sPinStatus == 1 || $user->sPinStatus == "1"){
					return $user;
				}
				if($user->sPin == $transkey){
					return $user;
				}
			}
...
function getSpent($period, $userId) {
	...
	//Generate Dynamic Safehaven Account
		
		public function generateDynamicSafeHaven($id){
			$id = (float) $id;
			$dbh=$this->connect();
	    	//$sql="SELECT * FROM subscribers WHERE sId = '$id' AND sDynamicBank = '' ";
	    	$sql="SELECT * FROM subscribers WHERE sId = $id";
			$queryC = $dbh->prepare($sql);
	    	$queryC->execute();
	      	$result=$queryC->fetch(PDO::FETCH_OBJ);
	      	...
	      	//Get User Details
		    	...
		    	if($result ==1 ){
		    	    $sql1="SELECT * FROM subscribers WHERE sId = '$id'";
        			$queryC1 = $dbh->prepare($sql1);
        	    	$queryC1->execute();
        	      	$result1=$queryC1->fetch(PDO::FETCH_OBJ);
				...
				if($aspfiyStatus == "On"){
					$obj->generateDynamicSafeHaven($id,'eq'.$result1->sEmail,$result1->sFname,$result1->sLname,$aspfiyApi,$result1->sPhone,$aspfiyWebhook);
				}
...
//Generate Billstack Accpount
		
		public function generateBillstack($id){
			...
			if(empty($result->sBillstackBank)){
				...
				$d=$this->getApiConfiguration();
				$bilstackStatus = $this->getConfigValue($d,"bilstackStatus");
				$billstackApi = $this->getConfigValue($d,"billstackApi");
				$billstackWebhook = $this->getConfigValue($d,"billstackWebhook");
				

				//If Billstack Is Active, Create Virtual Account For User
				if($bilstackStatus == "On"){
					$obj->generateBilstack($id,$result->sEmail,$result->sFname,$result->sLname,$billstackApi,$result->sPhone,$billstackWebhook);
				}
...
//----------------------------------------------------------------------------------------------------------------
	...
	//Generate paga Account
		...
		public function generatePaga($id){
			...
			if(empty($result->sPagaBank)){
				...
				//If paga Is Active, Create Virtual Account For User
				if($aspfiyStatus == "On"){
					$obj->generatePaga($id,$result->sEmail,$result->sFname,$result->sLname,$aspfiyApi,$result->sPhone,$aspfiyWebhook);
				}
