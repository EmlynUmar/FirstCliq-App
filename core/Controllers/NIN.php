    ...
    class NIN extends ApiAccess{
		...
		public function verifyMyNIN($body,$networkDetails){
            ...
            CURLOPT_HTTPHEADER => array(
                ...
                "Authorization: Bearer lv_aspget_b0w5tn18pvja665h74drog9sz9x43u1f"
            ),
            ));
            ...
            if($result->status==true || $result->status=='success'){
                ...
                // mysqli_connect removed to avoid hardcoded credentials
                ...
                $uslip = "https://agdatasub.com/slips/nin/{$slip}/?reportID=".$body->ref."&preview=1";
                $data = '
                {
                    "filepath":"'.$uslip.'",
                    "pagesize":"A4",
                ...
                if($respJ->convertedFilePath){
                    ...
                    curl_close ($ch);
                    fclose($fp);
                    $pdfURL = "https://agdatasub.com/slips/nin/{$slip}/".$pdfName;
                    
                    $dbh = $this->model->connect();
                    $stmt = $dbh->prepare("UPDATE reports SET pdf = :pdf WHERE transid = :ref");
                    $stmt->execute([':pdf' => $pdfURL, ':ref' => $body->ref]);
                }
            }
