	...
	class Account extends Model{
		...
		public function createVirtualBankAccount($id,$fname,$lname,$phone,$email,$monnifyApi,$monnifySecret,$monnifyContract){
			...
			if($value["requestSuccessful"] == true){
				...
				if($value["responseBody"]["accounts"][0]["bankCode"]== "035"){
					...
					$dbh=$this->connect();
					$c="UPDATE subscribers SET sBankName=:bn,sBankNo=:bno,accountReference=:ref WHERE sId=$id";
					$queryC = $dbh->prepare($c);
		...
		public function verifyAndUpdateNinOnMonnify($kycData){
			...
			if($shouldupdateStatus == "success"){
				...
				$servicedesc="NIN Verification with a charges fee of N$kycData->nincharges. Details Successfully Verified.";
				...
				$sql = "
					...
					SELECT @newwallet := @oldwallet - $kycData->nincharges;
				...
				$queryC->bindParam(':a',$kycData->nincharges,PDO::PARAM_STR);
